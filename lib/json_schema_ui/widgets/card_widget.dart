...
class _CardWidgetState extends State<CardWidget> {
  ...
  void initState() {
    ...
    sortedCardsByOrder = Utils().retrieveSchemaFields(schema: schema, uiSchema: uiSchema, context: context, path: path);
    index = 0;
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (schemaProperties[card]['properties'] != null) {
      cardFields = Utils().retrieveSchemaFields(schema: schemaProperties[card], uiSchema: uiSchema[card], context: context, path: path);
      hasCardFields = true;
      return Center(
          child: Row(
    ...
    return Container(
        ...
        child: hasCardFields
            ...
            itemBuilder: (BuildContext context, int i) {
              ...
              return ObjectBody(schema: schemaProperties[card], uiSchema: uiSchema[card], path: cardPropertyPath, field: cardProperty, disabled: false);
            })
            : ListView.builder(
            shrinkWrap: true,
            itemCount: cards.length,
            itemBuilder: (BuildContext context, int i) {
              return ObjectBody(schema: schema, uiSchema: uiSchema, path: path, field: card, disabled: false);
            })
    );
  }
}
